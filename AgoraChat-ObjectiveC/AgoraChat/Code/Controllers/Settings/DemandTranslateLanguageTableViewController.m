//
//  DemandTranslateLanguageTableViewController.m
//  AgoraChat-Demo
//
...
@interface DemandTranslateLanguageTableViewController ()
@property (nonatomic,strong) NSArray <AgoraChatTranslateLanguage *> *languages;
@property (nonatomic) NSInteger selectedItem;
@end

@implementation DemandTranslateLanguageTableViewController
...
- (void)viewDidLoad {
    ...
    [AgoraChatClient.sharedClient.chatManager fetchSupportedLanguages:^(NSArray<AgoraChatTranslateLanguage *> * _Nullable languages, AgoraChatError * _Nullable error) {
        if (!error) {
            ...
            for (AgoraChatTranslateLanguage* language in weakSelf.languages) {
                if ([ACDDemoOptions.sharedOptions.demandLanguage isEqualToString:language.languageCode]) {
    ...
    self.title = NSLocalizedString(@"demandLanguages", nil);
    // add done button
...
- (void)doneButtonAction:(UIButton*)button
{
    if (self.selectedItem != -1) {
        ACDDemoOptions.sharedOptions.demandLanguage = self.languages[self.selectedItem].languageCode;
