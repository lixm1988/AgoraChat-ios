...
{
    return [[ACDTextTranslationBubbleView alloc] initWithDirection:self.model.message.direction type:50 viewModel:[self translationsViewModel]];
}
...
- (void)translationsLabelTaped:(UITapGestureRecognizer*)gesture{
    switch(self.model.translateStatus) {
        case TranslateStatusSuccess:
            ...
            if ([self.bubbleView isKindOfClass:[ACDTextTranslationBubbleView class]]) {
                ACDTextTranslationBubbleView* bubbleView = (ACDTextTranslationBubbleView*)self.bubbleView;
                [bubbleView updateTextInfo];
            }
            [self updateTranslateInfo];
            break;
